...
$bal=(int)$row['balance']; if($bal<$amt) json_out(['allow'=>false,'msg'=>'insufficient']);
$new=$bal-$amt;
$pdo->beginTransaction();
try{
  $pdo->prepare('UPDATE cards SET balance=? WHERE uid=?')->execute([$new,$uid]);
  $pdo->prepare('INSERT INTO logs(uid,action,delta,balance_after)VALUES(?,?,?,?)')->execute([$uid],'spend',-$amt,$new);
  $pdo->commit();
